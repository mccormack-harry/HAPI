...
public class ActiveQuestsMenu implements QuestDisplayMenu {

    private final QuestManager manager;
    private final SmartInventory inventory;

    ActiveQuestsMenu(QuestManager manager) {
        this.manager = manager;
        inventory = SmartInventory.builder()
                .title(CCUtils.addColor("&6[Quests] &cActive Quests"))
                .size(3, 9)
                .provider(this)
                .build();
    }
    ...
    public void init(Player player, InventoryContents contents) {
        ...
        for (Quest quest: manager.getQuests(player, QuestStatus.ACTIVE)) {
            List<String> lore = new ArrayList<>();
            lore.add("&aProgress: &f" + quest.getProgress(player) + "&a/&f" + quest.getGoal());
            lore.addAll(quest.getFormattedDescription());
            lore.addAll(formatRewards(quest.getRewards()));
